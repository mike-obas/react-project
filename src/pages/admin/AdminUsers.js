...
function createData(name, email, role, created, action) {
  return {
    name,
    email,
    role,
...
export default function AdminUsers() {
  ...
  useEffect(() => {
    ...
    let cancel = false;
    axios
      .get("/getProducts/adminUsers/createdAt-desc")
      .then((res) => {
        if (cancel) return;
  ...
  useEffect(() => {
    let cancel = false;
    axios
      .get("/countQuery/adminUserCount/countField")
      .then((res) => {
        if (cancel) return;
  ...
  const handleGetMore = () => {
    ...
    axios
      .get(`/moreProducts/adminUsers/${lastId}/createdAt-desc`)
      .then((res) => {
        setLastId(res.data[0]);
        let newUsers = res.data;
        newUsers.shift();
  ...
  const handleEnable = (action) => () => {
    ...
    const values = {
      open: true,
      performAction: false,
      message: `Enable access to this admin officer`,
      actionText: 'Enable',
      cancelText: 'Cancel'
    }
  ...
  const handleDisable = (action) => () => {
    ...
    const values = {
      open: true,
      performAction: false,
      message: `This admin officer will be disabled until you reverse this action, do you wish to continue?` ,
      actionText: 'Disable',
      cancelText: 'Cancel'
    }
    return consumeContext.setDialog({ type: "open", modalContent: values })
  ...
  const handleDelete = (action) => () => {
    ...
    const values = {
      open: true,
      performAction: false,
      message: ` This admin officer will be deleted permanently, do you wish to continue?` ,
      actionText: 'Delete',
      cancelText: 'Cancel'
    }
    return consumeContext.setDialog({ type: "open", modalContent: values })
  ...
  useEffect(() => {
    ...
    if(
      ...
      uid.action === "disable"){
        ...
        axios.post("/userAccess/adminUsers", {uid: uid.id, action: true})
        .then(res => {
          if (cancel) return;
      ...
      uid.action === "enable"){
        setActionLoading(true)
        axios.post("/userAccess/adminUsers", {uid: uid.id, action: false})
        .then(res => {
          if (cancel) return;
          setActionLoading(false)
          setRerender(prevState => prevState +1)
     ...
     // console.log(`i am going to enable this ${uid.id}`) 
      ...
      uid.action === "delete"){
        setActionLoading(true)
        axios.post("/deleteUser/adminUsers/adminUserCount", {uid: uid.id})
        .then(res => {
          if (cancel) return;
          setActionLoading(false)
          setRerender(prevState => prevState +1)

...
//upload images on the fly: using onchange event
  ...
  .then(res => {
    ...
    if (localStorage.getItem("imagesForUpload")) {
      ...
      setTimeout(function(){ 
      setInput({ type: "setInput", field: e.target.name, fieldValue: res.data });
      }, 5000);
      localStorage.setItem("imagesForUpload", JSON.stringify(res.data));
    }
  })
  .catch(err => {
    ...
    promises
      ...
      .then(() => {
        if (
          ...
          //send post request
          axios.post("/manageProduct/add", sendValues)
          .then(res => {
            let modalContent = {
              open: true,
